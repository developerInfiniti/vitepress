...
При отсутствии названий кеша `WB` использует дефолтные названия, определенные в `workbox-core`. Предопределенные названия кеша выглядят так:
...
Теперь изображения будут записываться в кеш `my-image-cache`.
...
При использовании другой стратегии и получении непрозрачного ответа `WB` выведет в консоль предупреждение о том, что ответ не кеширован.
...
### Подготовка кеша во время выполнения
...
Запускам `wizard`. Он настраивает `CLI` для проекта, задавая несколько вопросов о его структуре для определения файлов для кеширования.
...
При обработке ответов на запросы к `URL`, начинающемуся с `/path/to/api/`, проверяется, присутствует ли в ответе заголовок `X-Is-Cacheable` (который добавляется сервером). Если заголовок присутствует и имеет значение `true`, такой ответ кешируется.
...
#### Обработка предварительно кешированных ответов
...
wb.addEventListener('activated', (event) => {
  ...
  if (!event.isUpdate) {
    ...
    // Если СВ настроен для предварительного кеширования ресурсов,
...
**Уведомление пользователя об обновлении кеша**
...
**Отправка СВ списка URL для кеширования**

В некоторых приложениях имеет смысл кешировать только те ресурсы, которые используются посещенной пользователем страницей. Модуль `workbox-routing` принимает список `URL` и кеширует их на основе правил, определенных в маршрутизаторе.

В приведенном ниже примере при каждой активации нового СВ в роутер отправляется список `URL` для кеширования. _Обратите внимание_, что мы можем отправлять все `URL`, поскольку будут кешированы только те из них, которые совпадут с роутами.
...
**Ручная реализация СВ, использующего стратегию "сначала кеш"**

```js
// Название кеша
// используется для обновления кеша
// в данном случае, для этого достаточно изменить версию кеша - my-cache-v2
...
// Предварительное кеширование ресурсов, выполняемое во время установки СВ
...
// Удаление старого кеша во время активации нового СВ
...
/*
  ...
  2. Если в кеше имеется ответ, он возвращается
  3. Если ответа в кеше нет, выполняется сетевой запрос
  4. Ответ на сетевой запрос кешируется и возвращается
  5. В кеш записываются только ответы на `GET-запросы`
